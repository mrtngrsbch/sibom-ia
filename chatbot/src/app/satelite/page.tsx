...
function SatelitePageContent() {
  console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Componente montado');
  ...
  const { analysis, setAnalysis, resetAnalysis, taskId: contextTaskId } = useSatelliteAnalysis();
  const [loading, setLoading] = useState(false);
  const [taskId, setTaskId] = useState<string | null>(null);

  console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Estado inicial:');
  console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - analysis (del contexto):', analysis);
  console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - taskId (del contexto):', contextTaskId);
  console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - taskId (local):', taskId);

  // Log de localStorage solo en el cliente
  useEffect(() => {
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] localStorage.getItem("satellite-analysis"):', localStorage.getItem('satellite-analysis'));
  }, []);
  ...
  useEffect(() => {
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] useEffect de polling ejecutado');
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] taskId (local):', taskId);
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] taskId (del contexto):', contextTaskId);

    if (!taskId) {
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] No hay taskId, polling no iniciado');
      return;
    }

    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Iniciando polling para taskId:', taskId);

    const pollInterval = setInterval(async () => {
      try {
        console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Polling - consultando estado del análisis...');
        const response = await client.getAnalysisStatus(taskId);
        console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Polling - response:', response);
        setAnalysis(response);

        if (response.status === 'completed' || response.status === 'failed') {
          console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Polling - análisis completado/fallido, deteniendo polling');
          clearInterval(pollInterval);
          setLoading(false);
        }
      } catch (error) {
        console.error('🔍 [SAT-DEBUG] [SatelitePageContent] Error en polling:', error);
        clearInterval(pollInterval);
        setLoading(false);
      }
    }, 2000);
    ...
    return () => {
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Cleanup - deteniendo polling');
      clearInterval(pollInterval);
    };
  }, [taskId]);

  const handleAnalyze = async (request: AnalyzeRequest) => {
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] handleAnalyze llamado');
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] request:', request);

    try {
      setLoading(true);
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] setLoading(true)');
      setAnalysis(null);
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] setAnalysis(null)');

      const response = await client.analyze(request);
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] client.analyze response:', response);

      setTaskId(response.task_id);
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] setTaskId(response.task_id):', response.task_id);
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] taskId (local):', taskId);
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] taskId (del contexto):', contextTaskId);
      ...
      console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Análisis inicializado con task_id:', response.task_id);
    } catch (error) {
      console.error('🔍 [SAT-DEBUG] [SatelitePageContent] Error iniciando análisis:', error);
      setAnalysis({
        task_id: '',
        partida: request.partida,
        status: 'failed',
  ...
  const handleReset = () => {
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] handleReset llamado');
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] Estado antes de reset:');
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - analysis (del contexto):', analysis);
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - taskId (del contexto):', contextTaskId);
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - taskId (local):', taskId);
    console.log('🔍 [SAT-DEBUG] [SatelitePageContent] - localStorage.getItem("satellite-analysis"):', localStorage.getItem('satellite-analysis'));
