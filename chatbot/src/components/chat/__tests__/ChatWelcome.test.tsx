...
describe("ChatWelcome", () => {
	...
	it("should render all FAQ questions", () => {
		...
		expect(
			screen.getByText("¿Cuáles municipios tienen información disponible?"),
		).toBeInTheDocument();
		expect(
			screen.getByText("¿Cómo busco una ordenanza específica?"),
		).toBeInTheDocument();
		expect(
			screen.getByText("¿Qué tipos de normativas puedo consultar?"),
		).toBeInTheDocument();
		expect(
			screen.getByText("¿Cómo cito una norma en mi búsqueda?"),
		).toBeInTheDocument();
	});
