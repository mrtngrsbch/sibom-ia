...
export function Sidebar({
	...
	const fetchStats = async (invalidateCache = false) => {
		try {
			...
			const url = invalidateCache ? `/api/stats?_=${Date.now()}` : "/api/stats";
	...
	const handleManualRefresh = async () => {
		...
		try {
			...
			if (reindexResponse.ok) {
				...
				console.log(
					"[Sidebar] Reindexación no disponible (probablemente en producción)",
				);
			}
		} catch (error) {
				...
				try {
					...
					if (response.ok) {
						...
						if (data.lastUpdated && data.lastUpdated !== lastKnownUpdate) {
							console.log(
								"[Sidebar] Detectado cambio en índice, recargando...",
							);
							await fetchStats(true);
						}
					}
				...
				<div className="px-3 py-2 text-sm text-slate-500 dark:text-slate-400 space-y-2">
					...
					<div className="flex items-center gap-2">
						...
						<span>
							{loading
								? "Cargando..."
								: `${stats?.totalDocuments.toLocaleString("es-AR") || 0} documentos`}
						</span>
					</div>
					{stats?.lastUpdated && (
