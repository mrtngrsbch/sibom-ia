...
function extractBalancePeriodFromQuery(query: string): string | null {
	...
	const ordinalTrimesterMatch = query.match(
		/\b(1|2|3|4)(?:er|ro|do|to)?\s*trimestre\b/i,
	);
	if (ordinalTrimesterMatch?.[1]) {
		return `${year}-T${ordinalTrimesterMatch[1]}`;
	}

	const wordTrimesterMatch = query.match(
		/\b(primer|segundo|tercer|cuarto)\s*trimestre\b/i,
	);
...
async function fetchGitHubIndex(): Promise<IndexEntry[]> {
	...
	try {
		...
		console.log(
			`[RAG] ✅ Índice descargado: ${data.length} documentos (${useGzip ? "gzip" : "sin comprimir"})`,
		);
		return data;
	} catch (error) {
...
async function fetchGitHubNormativasIndex(): Promise<NormativaIndexEntry[]> {
	...
	try {
		...
		console.log(
			`[RAG] ✅ Índice de normativas descargado: ${data.length} normativas`,
		);
		return data;
	} catch (error) {
		console.error(
			"[RAG] ❌ Error descargando índice de normativas de GitHub:",
			error,
		);
		throw error;
	}
...
async function hasNormativasIndexFileChanged(): Promise<boolean> {
	...
	try {
		...
		if (
			...
			fileModTime > normativasLastFileModTime
		) {
		...
		console.error(
			"[RAG] Error verificando cambios en índice de normativas:",
			error,
		);
		return false;
	}
...
async function loadNormativasIndex(): Promise<NormativaIndexEntry[]> {
	...
	console.log(
		`[RAG] 🔄 loadNormativasIndex() - useGitHub: ${useGitHub()}, cache size: ${normativasCache.length}`,
	);

	if (useGitHub()) {
		if (
			normativasCache.length > 0 &&
			now - normativasCacheTimestamp < CACHE_DURATION
		) {
		...
		if (fileChanged && normativasCache.length > 0) {
			console.log(
				`[RAG] 🔄 Detectado cambio en índice de normativas - Recargando...`,
			);
		}
	}
	...
	try {
		console.log(
			`[RAG] 📥 Cargando índice de normativas desde ${useGitHub() ? "GitHub" : "local"}...`,
		);
		...
		console.log(
			`[RAG] ✅ Índice de normativas cargado: ${normativasCache.length} normativas (fuente: ${useGitHub() ? "GitHub" : "local"})`,
		);
		return normativasCache;
	} catch (error) {
...
async function loadIndex(): Promise<IndexEntry[]> {
	...
	try {
		...
		console.log(
			`[RAG] ✅ Índice cargado: ${indexCache.length} documentos (fuente: ${useGitHub() ? "GitHub" : "local"})`,
		);
		return indexCache;
	} catch (error) {
...
async function retrieveContextFromNormativas(
	...
	if (normativas.length === 0) {
		console.log(
			"[RAG] ⚠️ Índice de normativas vacío, intentando fallback a boletines",
		);
		return retrieveContextFromBoletines(query, options);
	}

	console.log(
		`[RAG] 🔍 INICIO - Índice de normativas: ${normativas.length} registros`,
	);
	...
	if (options.municipality) {
		...
		console.log(
			`[RAG] 🏘️ Filtro municipio "${options.municipality}": ${filtered.length} normativas`,
		);
	}
	...
	if (options.type && options.type !== "all") {
		...
		console.log(
			`[RAG] 📋 Filtro tipo "${typeFilter}": ${filtered.length} normativas`,
		);
	}
	...
	if (options.dateFrom || options.dateTo) {
		...
		console.log(
			`[RAG] 📅 Filtro fecha: ${beforeSize} → ${filtered.length} normativas`,
		);
	}
	...
	if (options.type === "balances") {
		...
		if (balancePeriod) {
			...
			filtered = filtered.filter(
				(n) => (n.d || "").toLowerCase() === balancePeriod.toLowerCase(),
			);
			console.log(
				`[RAG] 🧾 Filtro período balance "${balancePeriod}": ${beforeSize} → ${filtered.length} normativas`,
			);
		}
	}
	...
	console.log(
		`[RAG] Índice BM25 construido con ${tokenizedDocs.length} normativas`,
	);
	...
	if (isMetadataOnly) {
		...
		if (resultNormativas.length > MAX_CONTEXT_ENTRIES) {
			...
			context =
				`LISTADO MASIVO: Se encontraron ${resultNormativas.length} normativas.
...
` +
			...
			context = resultNormativas
				.map(
					(n) =>
						`[${n.m}] ${n.t.toUpperCase()} N° ${n.n}/${n.y} - ${n.d} - ${n.ti}`,
				)
				.join("\n");
		}
	} else {
		...
		context = resultNormativas
			.map((n) => {
				...
				if (
					n.t === "balances" &&
					bulletinData.rag_chunks &&
					Array.isArray(bulletinData.rag_chunks) &&
					routingDecision
				) {
					console.log(
						`[RAG] 🎯 Using hierarchical chunks for Balance ${n.n}/${n.y}`,
					);
					console.log(
						`[RAG] Total chunks available: ${bulletinData.rag_chunks.length}`,
					);

					// Filtrar chunks por tier según routing decision
					const filteredChunks = filterChunksByTier(
						bulletinData.rag_chunks,
						routingDecision,
					);
					console.log(
						`[RAG] Chunks after tier filtering: ${filteredChunks.length} (tiers: ${routingDecision.tiers.join(", ")})`,
					);
					...
					if (filteredChunks.length > 0) {
						...
						const chunksContext = filteredChunks
							.map((chunk) => {
								...
								const completeness = chunk.completeness_score
									? `(${(chunk.completeness_score * 100).toFixed(0)}% complete)`
									: "";
...
Contenido (chunks jerárquicos):\n${chunksContext}...`;
				...
				const contentChunk = extractNormativaContent(
					fullContent,
					n.n,
					n.t,
					contentLimit,
...
Contenido: ${contentChunk}...`;
	...
	const contextEntries =
		isMetadataOnly && resultNormativas.length > MAX_CONTEXT_ENTRIES
			? MAX_CONTEXT_ENTRIES
			: resultNormativas.length;
	console.log(
		`[RAG] ✅ Query completada en ${duration}ms - ${resultNormativas.length} normativas recuperadas, ${contextEntries} en contexto`,
	);

	return {
		context:
			context || `No se encontró información específica para: "${query}"`,
		sources,
	};
}
...
async function retrieveContextFromBoletines(
	...
	console.log(
		`[RAG] 🔍 INICIO (legacy) - Índice total: ${index.length} documentos`,
	);
	...
	if (options.municipality) {
		...
		console.log(
			`[RAG] 🏘️ Filtro municipio "${options.municipality}": ${filteredIndex.length} docs restantes`,
		);
	}
	...
	if (
		options.type &&
		options.type !== "all" &&
		options.isManualTypeFilter === true
	) {
		...
		console.log(
			`[RAG] 📋 Filtro tipo MANUAL "${typeFilter}": ${filteredIndex.length} docs restantes`,
		);
	} else if (
		options.type &&
		options.type !== "all" &&
		...
		console.log(
			`[RAG] 📋 Tipo "${options.type}" detectado en query - buscando en contenido de boletines`,
		);
	}
	...
	if (options.dateFrom || options.dateTo) {
		...
		console.log(
			`[RAG] 📅 Filtro fecha: ${beforeSize} → ${filteredIndex.length} docs`,
		);
	}

	console.log(
		`[RAG] ✅ FINAL - Después de todos los filtros: ${filteredIndex.length} documentos`,
	);
		...
		filteredIndex.map(async (entry) => {
			try {
				...
				if (process.env.NODE_ENV !== "production") {
					console.warn(
						`[RAG] Error cargando ${(entry as any).sb || entry.filename}:`,
	...
	if (isComputational) {
		for (const doc of documents) {
			try {
				...
				console.warn(
					`[RAG] ⚠️ Error cargando tablas de ${doc.filename}:`,
					error,
				);
			}
		}
...
Contenido: ${contentChunk}...`;
	...
	const duration = Date.now() - startTime;
	console.log(
		`[RAG] Query "${query.slice(0, 30)}..." completada en ${duration}ms`,
	);
...
async function retrieveContextWithVectorSearch(
	...
	const vectorResults = await vectorSearch(query, {
		...
		type: options.type,
		year: options.dateFrom
			? parseInt(options.dateFrom.split("-")[0])
			: undefined,
		limit: options.limit || 10,
	});

	console.log(
		`[RAG] Vector search encontró ${vectorResults.length} resultados`,
	);
	...
	if (isMetadataOnly) {
		...
		context = validDocuments
			.map(
				(doc) => `[${doc.municipality}] ${doc.type.toUpperCase()} ${doc.number}
Título: ${doc.title}
Fecha: ${doc.date}
Estado: ${doc.status}`,
			)
...
Contenido: ${contentChunk}...`;
	...
	const duration = Date.now() - startTime;
	console.log(
		`[RAG] ✅ Vector search completado en ${duration}ms - ${validDocuments.length} docs`,
	);
...
export async function retrieveContext(
	...
	if (USE_NORMATIVAS_INDEX) {
		try {
			...
			console.error(
				"[RAG] ❌ Error con índice de normativas, usando fallback a boletines:",
				error,
			);
			console.error(
				"[RAG] Stack trace:",
...
export async function getDatabaseStats() {
		...
		try {
			...
			if (normativas.length > 0) {
				...
				try {
					if (useGitHub()) {
						lastUpdated =
							normativasCacheTimestamp > 0
								? new Date(normativasCacheTimestamp).toISOString()
								: null;
					} else {
						const basePath = getDataBasePath();
					...
					console.warn(
						"[RAG] No se pudo obtener fecha de actualización del índice de normativas",
					);
				}
			...
			console.error(
				"[RAG] Error cargando índice de normativas para stats, usando fallback:",
				error,
			);
		}
	}
	...
	try {
		if (useGitHub()) {
			...
			lastUpdated =
				cacheTimestamp > 0 ? new Date(cacheTimestamp).toISOString() : null;
		} else {
...
export function invalidateCache() {
	...
	console.log(
		"[RAG] 🔄 Cache invalidado completamente - se recargará en la próxima consulta",
	);
}
