...
const SAT_API_URL = process.env.NEXT_PUBLIC_SAT_API_URL || process.env.SAT_API_URL || 'http://localhost:8001';
...
export class SatAnalysisClient {
   ...
   */
  async analyze(request: AnalyzeRequest): Promise<{ task_id: string; status: TaskStatus; message: string }> {
