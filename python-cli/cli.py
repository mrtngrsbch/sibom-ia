...
try:
    ...
    HierarchicalChunker = None  # Fallback si el chunker no está disponible
    ...
    with ExecutionSummary("SIBOM", target) as summary:
        try:
            if args.all:
                ...
                result = scraper.run(
                    municipality=args.municipality, limit=args.limit)
                if result:
                    summary.add_success()
...
def cmd_transparency(args):
    ...
    async def run_transparency():
        ...
        if args.test_dir:
            ...
            console.print(
                "[yellow]MODO TEST: guardando en test/ (sin SQLite)[/yellow]")
        else:
        ...
        if large_pdfs_list.exists():
            ...
            if pending_lines:
                ...
                console.print(
                    f"   [dim]Para procesarlos: --url <url_del_primer_pdf> --pdf-engine vision-chunks[/dim]\n")
        ...
        if args.url:
            ...
            console.print(
                "[cyan]Modo: Automático (desde sources_user.yaml)[/cyan]")
            ...
            for source in config_data.get('sources', []):
                ...
                category_matches = (not source_categories) or (
            ...
            if not urls:
                console.print(
                    f"[yellow]No se encontraron URLs para {municipio} / {category}[/yellow]")
                console.print(
                    f"[yellow]Revisa {sources_file.name} o usa --url <URL>[/yellow]")
                return
        ...
        if limit < len(urls):
            console.print(
                f"[yellow]Limitando a {limit} de {len(urls)} URLs[/yellow]")
        ...
        if progress_file.exists():
            try:
                ...
                if content:
                    ...
                    console.print(
                        f"[dim]Retomando desde {len(processed_urls)} PDFs ya procesados[/dim]")
                else:
                    # Archivo vacío, empezar de cero
                    processed_urls = set()
            except (json.JSONDecodeError, Exception) as e:
                console.print(
                    f"[yellow]Advertencia: archivo de progreso corrupto, empezando de cero[/yellow]")
                processed_urls = set()
        ...
        try:
            ...
            summary_exec = ExecutionSummary(
                "Transparency", f"{municipio} - {category}")
            ...
            for i, url in enumerate(urls_to_process, 1):
                ...
                console.print(
                    f"[cyan][{i}/{len(urls_to_process)}] {url}[/cyan]")
                ...
                try:
                    async with httpx.AsyncClient(timeout=60.0) as client:
                        ...
                        if response.status_code != 200:
                            console.print(
                                f"[red]Error HTTP {response.status_code}[/red]")
                            error_count += 1
                            continue
                        ...
                        if is_large:
                            ...
                            console.print(
                                f"   [dim]Páginas:[/dim] {pdf_info['page_count']}")
                            console.print(
                                f"   [dim]Tamaño:[/dim] {pdf_info['size_mb']:.1f} MB")

                            if args.skip_large_pdfs:
                                console.print(
                                    f"   [red]→ OMITIENDO (agregado a lista manual)[/red]\n")

                                # Guardar en lista de PDFs para procesamiento manual
                                large_pdfs_list = output_dir / "_pdfs_grandes_pending.txt"
                                with open(large_pdfs_list, 'a') as f:
                                    f.write(
                                        f"{url} | {pdf_info['page_count']} pág | {pdf_info['size_mb']:.1f} MB | {filename}\n")

                                # Actualizar tracker
                                tracker.mark_skipped(
                                    url, f"PDF grande ({pdf_info['page_count']} pág)")

                                skip_count += 1
                                skip_this = True
                            else:
                                console.print(
                                    f"   [yellow]→ Procesando con {args.pdf_engine} engine...[/yellow]\n")
                                summary_exec.set_metric(
                                    "large_pdfs_processed", 1)
                        ...
                        if result:
                            ...
                            if is_transparency_doc and 'balance' in category.lower() and tablas_md:
                                try:
                                    ...
                                    for idx, md_table in enumerate(tablas_md):
                                        ...
                                        try:
                                            val_result = validator.validate_balance_sheet(
                                                df)
                                    ...
                                    if is_any_valid and not all_errors:
                                        ...
                                        console.print(
                                            f"[yellow]  ⚠ Validación fallida con Gemini. Re-intentando con GLM-OCR...[/yellow]")
                                        try:
                                            ...
                                            if glm_result:
                                                ...
                                                g_tablas_md = extract_tables_as_markdown(
                                                    g_content)
                                                ...
                                                for md_table in g_tablas_md:
                                                    df = markdown_to_dataframe(
                                                        md_table)
                                                    if df.empty:
                                                        continue
                                                    v_res = validator.validate_balance_sheet(
                                                        df)
                                                    if v_res['is_valid'].all():
                                                        g_is_any_valid = True
                                                    else:
                                                        g_all_errors.extend(
                                                            [f"GLM Row Error" for _ in range(len(v_res[~v_res['is_valid']]))])

                                                if g_is_any_valid and not g_all_errors:
                                                    console.print(
                                                        f"[green]  ✓ GLM-OCR solucionó la validación![/green]")
                                                    ...
                                                    console.print(
                                                        f"[dim]  → GLM-OCR también falló o no mejoró la validación.[/dim]")
                                        except Exception as fallback_e:
                                            console.print(
                                                f"[dim]  → Error en fallback GLM-OCR: {fallback_e}[/dim]")
                            ...
                            try:
                                cabecera, periodo_from_header = extract_header_with_periodo_code(
                                    content, municipio)

                                if cabecera.get("fecha_generacion"):
                                    # Formato: "20/07/2020 08:09" -> tomar solo la fecha
                                    fecha_doc_extraida = cabecera["fecha_generacion"].split()[
                                        0]
                            ...
                            if periodo_extraido == "s/d":
                                ...
                                trimestre_match = re.search(
                                    r'(\d+)[°o]?\s*-?\s*Trimestre', filename, re.IGNORECASE)
                                semestre_match = re.search(
                                    r'(\d+)[°o]?\s*-?\s*Semestre', filename, re.IGNORECASE)

                                if year_match:
                                    year = year_match.group(0)
                                    if trimestre_match:
                                        trimestre_num = trimestre_match.group(
                                            1)
                                ...
                                try:
                                    ...
                                    for md_table in tablas_md:
                                        ...
                                        if not df.empty:
                                            ...
                                            table_chunks = chunker.generate_chunks(
                                                df, meta)
                                            rag_chunks.extend(table_chunks)

                                    if rag_chunks:
                                        console.print(
                                            f"[green]  ✓ Generados {len(rag_chunks)} chunks para el RAG[/green]")
                                except Exception as chunk_err:
                                    console.print(
                                        f"[yellow]  ⚠ Error generando chunks: {chunk_err}[/yellow]")
                            ...
                            if category.lower() == "balances" and BalanceExtractor is not None:
                                try:
                                    ...
                                    if summary.is_complete:
                                        ...
                                        console.print(
                                            f"[green]  ✓ Resumen numérico: {summary.campos_extraidos}/7 campos (completo)[/green]")
                                    elif summary.campos_extraidos > 0:
                                        doc_data['resumen_ejecutivo_numerico'] = summary.to_dict(
                                        )
                                        console.print(
                                            f"[yellow]  ⚠ Resumen parcial: {summary.campos_extraidos}/7 campos[/yellow]")
                                    else:
                                        console.print(
                                            f"[dim]  • Sin resumen numérico (0 campos extraídos)[/dim]")
                                except Exception as e:
                                    console.print(
                                        f"[yellow]  ⚠ Error extrayendo resumen: {e}[/yellow]")
                            ...
                            if category.lower() == "balances" and HierarchicalChunker is not None:
                                try:
                                    chunker = HierarchicalChunker(
                                        verbose=False)
                                    hierarchical_chunks = chunker.chunk_balance(
                                        doc_data)

                                    if hierarchical_chunks:
                                        # Convertir a formato compatible (lista de dicts)
                                        doc_data['rag_chunks'] = [
                                            chunk.to_dict() for chunk in hierarchical_chunks]
                                        doc_data['rag_chunks_count'] = len(
                                            hierarchical_chunks)
                                    ...
                                    console.print(
                                        f"[yellow]  ⚠ Error generando chunks jerárquicos: {e}[/yellow]")
                            ...
                            if not args.test_dir:
                                try:
                                    ...
                                    console.print(
                                        f"[dim]  → SQLite actualizado[/dim]")
                                except Exception as sqlite_err:
                                    console.print(
                                        f"[yellow]  ⚠ SQLite: {sqlite_err}[/yellow]")
                            else:
                                console.print(
                                    f"[dim]  → MODO TEST: SQLite omitido[/dim]")
                            ...
                            if content and len(content) > 1000:
                                ...
                                console.print(
                                    f"[yellow]  ⚠ Contenido vacío/corto, NO se guarda progreso[/yellow]")
                            ...
                            tracker.mark_processed(
                                url=url,
                                model=quality.get(
                                    'extraction_method', args.pdf_engine),
                            ...
                            success_count += 1
                            console.print(
                                f"[green]✓ Extraído: {title[:50]}...[/green]")
                            console.print(
                                f"  Tablas: {len(tablas_md)}, Calidad: {quality['confidence']:.1%}")
                            console.print(
                                f"  [dim]JSON guardado: {output_file.name}[/dim]")
                        else:
                            error_count += 1
                            console.print(
                                f"[yellow]⚠ No se pudo extraer contenido[/yellow]")
                            tracker.mark_error(
                                url, "No se pudo extraer contenido")
                    ...
                    save_progress()
                    console.print(
                        f"[green]✓ Progreso guardado: {len(processed_urls)} PDFs[/green]")
                    break
                    ...
                    console.print(
                        "  3. Vuelve a ejecutar el mismo comando para continuar")
            ...
            if processed_urls and success_count > 0:
                console.print(
                    f"[green]✓ Progreso guardado en {progress_file.name}[/green]")
                console.print(
                    "[dim]Usa el mismo comando para retomar donde dejaste[/dim]")
    ...
    try:
        ...
        console.print(
            "[dim]El progreso se guardó hasta el último PDF exitoso[/dim]")
...
def cmd_db(args):
    ...
    with ExecutionSummary("SQLite Manager", action, show_startup=False) as summary:
        ...
        if args.stats:
            console.print(
                "[yellow]Estadísticas de la base de datos:[/yellow]\n")
            mgr.print_stats()
            ...
            if results:
                ...
                for r in results[:args.limit]:
                    table.add_row(
                        ...
                        r['year'],
                        r['date'] or "-",
                        r['title'][:40] + "..." if r['title'] and len(
                            r['title']) > 40 else (r['title'] or "-")
                    )
    ...
    async def run_vision(summary: ExecutionSummary):
        ...
        try:
            ...
            if result:
                ...
                console.print(
                    Panel.fit(f"[bold green]Extracción exitosa[/bold green]"))
                console.print()
                ...
                if args.output:
                    ...
                    with output_path.open('w', encoding='utf-8') as f:
                        f.write(content)
                    console.print(
                        f"[green]✓ Contenido guardado en {output_path}[/green]")
...
def cmd_index(args):
    ...
    with ExecutionSummary("Index Manager", "Rebuild", show_startup=False) as summary:
        if args.rebuild:
            console.print(
                "[yellow]Reconstruyendo índices desde boletines/ y SQLite...[/yellow]\n")
            ...
            for json_file in BOLETINES_DIR.glob("*.json"):
                ...
                try:
                    ...
                    console.print(
                        f"[yellow]  Error leyendo {json_file.name}: {e}[/yellow]")
            ...
            console.print(
                "[cyan]2. Leyendo documentos de transparencia desde SQLite...[/cyan]")
            try:
                from utils.sqlite_manager import get_sqlite_manager
                mgr = get_sqlite_manager()
                ...
                console.print(
                    f"[cyan]  Documentos de transparencia: {len(trans_docs):,}[/cyan]\n")
                all_normas.extend(trans_docs)

            except Exception as e:
                console.print(
                    f"[yellow]  ⚠ No se pudieron leer documentos de transparencia: {e}[/yellow]\n")

            console.print(
                f"[bold green]Total de documentos a indexar: {len(all_normas):,}[/bold green]\n")
...
def cmd_scrape(args):
    ...
    console.print(
        "\n[bold cyan]╔════════════════════════════════════════╗[/bold cyan]")
    console.print(
        "[bold cyan]║         SIBOM SCRAPER v2.0              ║[/bold cyan]")
    console.print(
        "[bold cyan]╚════════════════════════════════════════╝[/bold cyan]\n")
    console.print(f"[dim]Municipio:[/dim] {municipio}")
    console.print(f"[dim]Categoría:[/dim] {categoria}")
    console.print(
        f"[dim]Guardar PDFs:[/dim] {'Sí' if args.save_pdfs else 'No'}")
    ...
    try:
        ...
        console.print(
            "[yellow]El progreso se guardó. Usa --resume para continuar.[/yellow]")
    except Exception as e:
        console.print(f"\n[red]❌ Error: {e}[/red]")
        import traceback
        traceback.print_exc()
...
def cmd_status(args):
        ...
        for source in sources:
            ...
            for cat in cats:
                if cat not in categories_info:
                    categories_info[cat] = {
                        'total_urls': 0, 'enabled': False, 'sources': []}
        ...
        for progress_file in info['progress_files']:
            try:
                with progress_file.open('r') as f:
                    urls = json.load(f)
                    processed_urls += len(urls) if isinstance(urls,
                                                              list) else 0
            except:
                pass

        # Total URLs para este municipio
        total_urls = sum(v['total_urls'] for k, v in categories_info.items(
        ) if name.lower() in ' '.join(v['sources']).lower())
    ...
    if categories_info:
        ...
        for cat, info in sorted(categories_info.items()):
            ...
            sources_str = ', '.join(info['sources'])[
                :40] + '...' if len(', '.join(info['sources'])) > 40 else ', '.join(info['sources'])
    ...
    try:
        ...
        if stats:
            ...
            estimated_cost_usd = (
                total_tokens_estimated / 1_000_000) * cost_per_million_tokens
            ...
            api_table.add_row(
                "Restantes", f"[{bar_color}]{remaining}[/{bar_color}]")
            api_table.add_row(
                "Progreso", f"[{bar_color}][{bar}][/{bar_color}] {percent:.1f}%")
            api_table.add_row("")
            api_table.add_row("Calls hist.", f"[dim]{total_all_time:,}[/dim]")
            api_table.add_row(
                "Tokens est.", f"[dim]~{total_tokens_estimated:,}[/dim]")
            api_table.add_row(
                "Costo total", f"[dim]~${estimated_cost_usd:.2f} USD[/dim]")
    ...
    try:
        ...
        console.print(
            "[dim]  📝 Editá config/models.yaml para cambiar modelos[/dim]")
        console.print()
    except Exception as e:
    ...
    try:
        ...
        if stats and stats.get('models'):
            ...
            for model_name, data in stats['models'].items():
                ...
                short_name = data.get(
                    'display_name', model_name.split('/')[-1][:20])
            ...
            total = stats['total']
            console.print(
                f"  [dim]Total: {total['calls']} calls, {total['total_tokens']:,} tokens, ${total['cost']:.4f} USD[/dim]")
    ...
    try:
        ...
        if stats.get('total_normativas', 0) > 0 or stats.get('total_transparency', 0) > 0:
            ...
            db_table.add_row(
                "Transparencia", f"[bold green]{total_trans:,}[/bold green]")
            db_table.add_row(
                "  Con tablas", f"[dim]{trans_with_tables:,}[/dim]")

            # Total
            db_table.add_row("")
            db_table.add_row("Total documentos",
                             f"[bold]{total_norm + total_trans:,}[/bold]")
    ...
    try:
        ...
        for municipio_dir in BOLETINES_DIR.iterdir():
            if municipio_dir.is_dir() and not municipio_dir.name.startswith('.') and municipio_dir.name not in IGNORED_DIRS:
                ...
                quality_stats[m_name] = {
                    "valid": 0, "failed": 0, "error": 0, "unchecked": 0, "tables": 0}
                ...
                for json_file in municipio_dir.rglob("*.json"):
                    try:
                        ...
                        quality_stats[m_name]["tables"] += len(
                            data.get("tablas_md", []))
                    except:
                        pass
        ...
        for m_name, q_info in sorted(quality_stats.items()):
            ...
            status_summary = []
            if q_info["valid"] > 0:
                status_summary.append(f"[green]{q_info['valid']} ✅[/green]")
            if q_info["failed"] > 0:
                status_summary.append(f"[red]{q_info['failed']} ❌[/red]")
            if q_info["error"] > 0:
                status_summary.append(f"[yellow]{q_info['error']} ⚠[/yellow]")

            quality_table.add_row(
                m_name,
                " | ".join(
                    status_summary) if status_summary else "[dim]—[/dim]",
        ...
        if quality_table.row_count > 0:
            ...
            console.print(
                "[dim]  No hay datos de validación disponibles aún.[/dim]")
    ...
    try:
                    ...
                    try:
                        ...
                        for file_id, info in proc_data.get("files", {}).items():
                            if info.get("status") == "skipped":
                                url = info.get("url", "")
                                filename = url.split(
                                    '/')[-1] if not info.get("filename") else info.get("filename")
        ...
        if skipped_files:
            console.print(
                "[bold yellow]⚠️ Archivos Grandes (Pendientes):[/bold yellow]")
            ...
            for f in skipped_files:
                skipped_table.add_row(
                    f["municipio"], f["archivo"][:40], f["razon"])
